...
import { AptosClient, AptosAccount, FaucetClient, BCS, TxnBuilderTypes, Types, MaybeHexString, HexString } from "aptos";
import { sha3_256 as sha3Hash } from "@noble/hashes/sha3";
import { FAUCET_URL, NODE_URL, fungibleStore } from "./common";
import assert from "assert";

const { AccountAddress, EntryFunction, MultiSig, MultiSigTransactionPayload, TransactionPayloadMultisig } = TxnBuilderTypes;


// Step 0: After publish the example code with the example code under any `MODULE_ADDR` using CLI or any sdk.
...
(async () => {
  ...
  const createMultisigManagedCoin = await client.generateTransaction(owner1.address(), {
    ...
    type_arguments: [],
    arguments: [[owner2.address().hex(), owner3.address().hex()], 2, ["description"], [BCS.bcsSerializeStr("The multisig account for Meme")], 0, "meme coin", ASSET_SYMBOL, 8, "http://meme.xyz/favicon.ico", "http://meme.xyz", [true, true, true]],
  });
  // <:!:section_2
  await client.generateSignSubmitWaitForTransaction(owner1, createMultisigManagedCoin.payload);
  ...
  // Create the mint transaction payload
  {
    ...
    BCS.serializeVector([AccountAddress.fromHex(owner2.address()), AccountAddress.fromHex(owner3.address())], recipientsSerializer);
    const mintTxPayload = new MultiSigTransactionPayload(
      EntryFunction.natural(
        MODULE_ADDR + "::managed_fungible_asset",
        "mint_to_primary_stores",
        [],
        [BCS.bcsToBytes(metadata), recipientsSerializer.getBytes(), BCS.serializeVectorWithFunc([1_000, 2_000], "serializeU64")],
      ),
    );
    ...
    let fungibleStore2 = await client.getAccountResource(getPrimaryStoreAddress(owner2, metadata), fungibleStore);
    assert(parseInt((fungibleStore2?.data as any).balance) === 1_000);
    let fungibleStore3 = await client.getAccountResource(getPrimaryStoreAddress(owner3, metadata), fungibleStore);
    assert(parseInt((fungibleStore3?.data as any).balance) === 2_000);
  }
  ...
  // Create the reeze transaction payload
  {
    ...
    // <:!:section_4
    let fungibleStore1 = await client.getAccountResource(getPrimaryStoreAddress(owner1, metadata), fungibleStore);
    assert(Boolean((fungibleStore1?.data as any).frozen));
  }
  ...
  // ===========================================================================================
  {
    ...
    const transferPayload = new MultiSigTransactionPayload(
      EntryFunction.natural(
        ...
        [],
        [BCS.bcsToBytes(metadata), transferSendersSerializer.getBytes(), transferRecipentsSerializer.getBytes(), BCS.serializeVectorWithFunc([1_000], "serializeU64")],
      ),
    );
    ...
    await client.generateSignSubmitWaitForTransaction(owner2, transferTx.payload);
    ...
    let fungibleStore1 = await client.getAccountResource(getPrimaryStoreAddress(owner1, metadata), fungibleStore);
    assert(parseInt((fungibleStore1?.data as any).balance) === 1_000);
    let fungibleStore3 = await client.getAccountResource(getPrimaryStoreAddress(owner3, metadata), fungibleStore);
    assert(parseInt((fungibleStore3?.data as any).balance) === 1_000);
  }
  ...
  // ===========================================================================================
  {
    ...
    BCS.serializeVector([AccountAddress.fromHex(owner1.address()), AccountAddress.fromHex(owner2.address()), AccountAddress.fromHex(owner3.address())], burnAccountsSerializer);
    const burnPayload = new MultiSigTransactionPayload(
      EntryFunction.natural(
        MODULE_ADDR + "::managed_fungible_asset",
        "burn_from_primary_stores",
        [],
        [BCS.bcsToBytes(metadata), burnAccountsSerializer.getBytes(), BCS.serializeVectorWithFunc([1_000, 1_000, 1_000], "serializeU64")],
      ),
    );
    ...
    let fungibleStore1 = await client.getAccountResource(getPrimaryStoreAddress(owner1, metadata), fungibleStore);
    assert(parseInt((fungibleStore1?.data as any).balance) === 0);
    let fungibleStore2 = await client.getAccountResource(getPrimaryStoreAddress(owner2, metadata), fungibleStore);
    assert(parseInt((fungibleStore2?.data as any).balance) === 0);
    let fungibleStore3 = await client.getAccountResource(getPrimaryStoreAddress(owner3, metadata), fungibleStore);
    assert(parseInt((fungibleStore3?.data as any).balance) === 0);
  }
})();
...
const getPrimaryStoreAddress = (owner: AptosAccount, metadata: TxnBuilderTypes.AccountAddress): HexString => {
  const hash = sha3Hash.create();
  hash.update(BCS.bcsToBytes(AccountAddress.fromHex(owner.address())));
  hash.update(BCS.bcsToBytes(metadata));
  hash.update(new Uint8Array([0xFC]));
  return HexString.fromBuffer(Buffer.from(hash.digest()));
};
